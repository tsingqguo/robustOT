...
from LRR.models import make as make_model
from LRR.models.liif import STIR
from LRR.utils import make_coord
from typing import NamedTuple, Optional


def make_stir(model_spec: dict, args: Optional[dict] = None) -> STIR:
...
def make_rsn(model_spec: dict) -> ResampleCNN | ResampleMLP:
...
class ResampleMLPArgs(NamedTuple):
    feat_dim: int
    n_length: int
...
class ResampleCNNArgs(NamedTuple):
    feat_dim: int
    n_length: int
    layers: list[int]
    text_dim: int
...
class LRR(nn.Module):
    stir: STIR
    rsn: Optional[ResampleCNN | ResampleMLP]
    device: torch.device
    coord: torch.Tensor
    cell: torch.Tensor
    ...
    def forward(self, x: torch.Tensor, text: Optional[str] = None):
