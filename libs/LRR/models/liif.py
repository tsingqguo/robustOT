...
from . import make, register
from .edsr import EDSR
from .mlp import MLP
from LRR.utils import make_coord


def make_encoder(encode_spec) -> EDSR:
    return make(encode_spec)  # type: ignore


def make_imnet(imnet_spec, args) -> MLP:
    return make(imnet_spec, args=args)  # type: ignore
...
class STIR(nn.Module):
    ...
    def __init__(
        ...
        if imnet_spec is not None:
            ...
            if self.feat_unfold:
                self.multiplier = (
                    self.feat_unfold_kernel * self.feat_unfold_kernel
                )
    ...
    def query_rgb_2t3(
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                rel_coord[:, :, 0] *= feat.shape[-2]  # ratio->pixel
