...
from attackers.csa.CSA.data_utils import normalize
...
def adv_attack_template(
    img_tensor: torch.Tensor, GAN: Base_L2_500_Template
) -> torch.Tensor:
    """adversarial attack to template"""
    """input: pytorch tensor(0,255) ---> output: pytorch tensor(0,255)"""
    """step1: Normalization"""
...
def adv_attack_template_S(
    img_tensor: torch.Tensor, GAN: Base_L2_500_Search, target_sz=(127, 127)
) -> torch.Tensor:
    """adversarial attack to template"""
    """input: pytorch tensor(0,255) ---> output: pytorch tensor(0,255)"""
    """step1: Normalization"""
...
def adv_attack_search(
    img_tensor: torch.Tensor, GAN: Base_L2_500_Search, search_sz=(255, 255)
) -> torch.Tensor:
...
_TType = Literal["tensor", "array"]
"""`\"tensor\"` or `\"array\"`"""


def get_subwindow(
    im: np.ndarray,
    pos: Union[npt.NDArray[np.float64], float, List[float]],
    model_sz: int,
    original_sz: int,
    avg_chans: npt.NDArray[np.float64],
    type: _TType = "tensor",
) -> Union[npt.NDArray[np.float64], torch.Tensor]:
    """
    args:
        im: bgr based image
        pos: center position
    ...
    return im_patch  # type: ignore


def multi_cropx(
    img, center_pos, size, channel_average, type: _TType = "tensor"
):
    """crop search region"""
    """pos:(N,2); size:(N,2)"""
    ...
    for i in range(s_x.shape[0]):
        x_crop[i] = get_subwindow(
            img, center_pos[i], 512, s_x[i], channel_average, type
        )
    return x_crop, scale_x


def add_gauss_noise(input: torch.Tensor, sigma: float):
...
def generate_impulse_mask(im_sz: torch.Size, prob: float):
...
def add_pulse_noise(input: torch.Tensor, prob: float):
