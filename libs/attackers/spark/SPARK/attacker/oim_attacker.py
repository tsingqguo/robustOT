...
class OIMAttacker():
    atk_type: Literal["TA", "UA"]
    eplison: int
    inta: int
    norm_type: Literal["L_1", "L_2", "L_inf", "Momen"]
    reg_type: Literal["L21", "L2", "L_inf", "weighted"]
    ...
    def __init__(self, atk_type, max_num=10, eplison=1, inta=10, lamb=0.00001, norm_type='L_inf', apts_num=2, reg_type='weighted',accframes=30):
    ...
    def attack(
        ...
        """
        print("called attack")
        w_z: np.float64 = tracker.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(tracker.size)
        h_z: np.float64 = tracker.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(tracker.size)
        s_z: np.float64 = np.sqrt(w_z * h_z)
        scale_z: np.float64 = cfg.TRACK.EXEMPLAR_SIZE / s_z
        s_x:np.float64 = s_z * (cfg.TRACK.INSTANCE_SIZE / cfg.TRACK.EXEMPLAR_SIZE)
        ...
        if prev_perts is None or (same_prev == False and Enable_same_prev == True):
            ...
            if APTS == False:
                ...
                if cfg.ATTACKER.GEN_ADV_IMG:
                    adv_img = tracker.get_origin_img(img, x_crop, tracker.center_pos,
                                                 cfg.TRACK.INSTANCE_SIZE,
                                                 round(s_x), tracker.channel_average)
    ...
    def ua_label(self, tracker: BaseTracker, scale_z: np.float64, outputs: _MB_Track_R):
    ...
    def ta_label(self, tracker: BaseTracker, scale_z: np.float64, outputs: _MB_Track_R):
    ...
    def oim_once(self, tracker: BaseTracker, data: AtkData):
        ...
        if self.reg_type == 'L21':
            reg_loss = torch.linalg.norm(t_prev_perts, 2, 1).sum()  # +torch.norm(pert,2)
        elif self.reg_type == 'L2':
            reg_loss = torch.linalg.norm(t_prev_perts, 2)
            ...
            reg_loss = torch.linalg.norm(t_prev_perts, 2, 1).sum()
        else:
            reg_loss = 0.
        ...
        if self.norm_type == 'L_inf':
            ...
            x_grad = x_grad / torch.linalg.norm(x_grad)
            adv_x = adv_x + pert + pert_sum  + self.eplison * x_grad
            pert = adv_x - search-pert_sum
            pert = torch.clamp(pert / torch.linalg.norm(pert), -self.inta, self.inta)
        elif self.norm_type=='Momen':
            momen = self.lamb_momen*momen+x_grad/torch.linalg.norm(x_grad,1)
