...
'''siamrpn++'''
from attackers.csa.CSA.siamRPNPP import SiamRPNPP
...
class GsearchL2500regressModel(BaseModel_CSA):
    ...
    def forward(self,target_sz=(255,255)):
        ...
        '''resize to (512,512)'''
        search512_clean1 = F.interpolate(self.search_clean1, size=(512, 512), mode='bilinear',align_corners=False)
        search512_adv1 = search512_clean1 + self.netG(search512_clean1)  # Residual form: G(A)+A
        '''Then crop back to (255,255)'''
        self.search_adv1 = F.interpolate(search512_adv1, size=target_sz, mode='bilinear',align_corners=False)
    ...
    def transform(self,patch_clean1,target_sz=(255,255)):
        '''resize to (512,512)'''
        patch512_clean1 = F.interpolate(patch_clean1, size=(512, 512), mode='bilinear',align_corners=False)
        patch512_adv1 = patch512_clean1 + self.netG(patch512_clean1)  # Residual form: G(A)+A
        patch_adv1 = F.interpolate(patch512_adv1, size=target_sz, mode='bilinear',align_corners=False)
