...
from pysot.utils.bbox import Corner, IoU, corner2center
...
class AnchorTarget:
    def __init__(
        self,
        im_c_overwrite: Optional[int] = None,
        size_overwrite: Optional[int] = None,
    ):
        self.anchors = Anchors(
            CFG.anchor.stride, CFG.anchor.ratios, CFG.anchor.scales
        )

        if im_c_overwrite is not None:
            im_c = im_c_overwrite
        else:
            im_c = CFG.train.search_size // 2

        if size_overwrite is not None:
            size = size_overwrite
        else:
            size = CFG.train.output_size

        self.anchors.generate_all_anchors(im_c=im_c, size=size)

    def __call__(self, target: Corner, size: int, neg=False):
        ...
        if neg:
            ...
            cx += int(
                np.ceil(
                    (tcx - CFG.train.search_size // 2) / CFG.anchor.stride
                    + 0.5
                )
            )
            cy += int(
                np.ceil(
                    (tcy - CFG.train.search_size // 2) / CFG.anchor.stride
                    + 0.5
                )
            )
            l = max(0, cx - 3)
        ...
        # print('\n[debug] AnchorTarget: delta.shape =', list(delta.shape))
        # print('[debug]                  cx.shape =', list(cx.shape))
