import cv2
import numpy as np
from pyotp.utils import add_gaussian_noise
...
class Augmentation:
    ...
    def _flip_aug(self, image, bbox):
        ...
        width = image.shape[1]
        bbox = Corner(
            width - 1 - bbox.x2, bbox.y1, width - 1 - bbox.x1, bbox.y2
        )
        return image, bbox
    ...
    def __call__(
        ...
        # gaussian noise but not augmentation
        if self.gn_var > 0:
            image = add_gaussian_noise(image, self.gn_mean, self.gn_var)
