...
class DistModule(nn.Module, Generic[M]):
    module: M

    def __init__(self, module: M, bn_method: int = 0):
...
def broadcast_params(model: nn.Module):
    """ broadcast model parameters """
    for p in model.state_dict().values():
        dist.broadcast(p, 0)
    # for name, p in model.state_dict().items():
    #     print(f"dist.broadcasting {name}")
    #     dist.broadcast(p, 0)


def broadcast_buffers(
    model: nn.Module,
    method: Literal[0, 1, 2] = 0
):
...
def dist_init():
    ...
    try:
        ...
        if 'public' in e.args[0]:
            logger.warning(e)
            logger.warning('Warning: use single process')
            rank, world_size = 0, 1
        else:
...
def reduce_gradients(model, _type: Literal['avg','sum']='sum'):
    types = ['sum', 'avg']
    assert _type in types, f'gradients method must be in "{types}"'
    logger.info(f"gradients method is {_type}", extra={'once': True})
    if get_world_size() > 1:
