import math
import numpy as np
from pyotp.config.pysot import (
    CFG,
    _Cfg_LrType,
    _Cfg_Train_Lr,
    _Cfg_Train_Lr_Warmup,
)
...
class WarmUPScheduler(LRScheduler):
    def __init__(
        ...
        warmup_lr = warmup.lr_spaces  # [::-1]
        normal_lr = normal.lr_spaces
        self.lr_spaces = np.concatenate([warmup_lr, normal_lr])
        self.start_lr = normal_lr[0]

        super(WarmUPScheduler, self).__init__(optimizer, last_epoch)


LRs: Dict[_Cfg_LrType, Type[LRScheduler]] = {
    "log": LogScheduler,
    "step": StepScheduler,
...
def _build_lr_scheduler(
    optimizer: Optimizer,
    config: Union[_Cfg_Train_Lr, _Cfg_Train_Lr_Warmup],
    epochs: int = 50,
    last_epoch: int = -1,
) -> LRScheduler:
...
def _build_warm_up_scheduler(optimizer: Optimizer, epochs=50, last_epoch=-1):
    warmup_epoch = CFG.train.lr.warmup.epoch
    sc1 = _build_lr_scheduler(
        optimizer, CFG.train.lr.warmup, warmup_epoch, last_epoch
    )
...
def build_lr_scheduler(optimizer: Optimizer, epochs=50, last_epoch=-1):
    if CFG.train.lr.warmup.warmup:
