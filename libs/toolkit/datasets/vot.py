...
import json
import numpy as np
from .dataset import Dataset, VariantNotFound
...
ValidDatasetNames = Literal["VOT2016", "VOT2017", "VOT2018", "VOT2019"]
ValidDatasetNames_LT = Literal["VOT2018-LT"]


class VOTVideo(Video[float]):
    gt_rect: Any
    ...
    def __init__(
        ...
        self.tags["empty"] = (
            np.all(1 - np.array(all_tag), axis=1).astype(np.int32).tolist()
        )
        ...
        if not load_img:
            img_name = os.path.join(
                os.path.abspath(root), os.path.abspath(self.img_names[0])
            )
    ...
    def load_tracker(
        ...
        """
        tracker_names, variant = self._prepare_tracker_names(path, tracker_names, variant)
        if not os.path.isdir(os.path.join(path, tracker_names[0], variant)):
            raise VariantNotFound
        for name in tracker_names:
            traj_files = glob(
                os.path.join(path, name, variant, self.name, "*0*.txt")
            )
...
class VOTDataset(Dataset[VOTVideo]):
    def __init__(
        ...
        super().__init__(name, dataset_root, VOTVideo)
        with open(os.path.join(dataset_root, name + ".json"), "r") as f:
            meta_data: dict[str, Any] = json.load(f)

        # load videos
        p_bar = tqdm(meta_data.keys(), desc="loading " + name)
        self.videos = {}
        for video in p_bar:
            video: str
            p_bar.set_postfix_str(video)
...
class VOTLTVideo(Video):
    ...
    def __init__(
        ...
        self.gt_traj = [
            [0] if np.isnan(bbox[0]) else bbox for bbox in self.gt_traj
        ]
        if not load_img:
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        for name in tracker_names:
            traj_file = os.path.join(
                path, name, "longterm", self.name, self.name + "_001.txt"
            )
            with open(traj_file, "r") as f:
                traj = [
                    list(map(float, x.strip().split(",")))
...
class VOTLTDataset(Dataset[VOTLTVideo]):
    def __init__(
        self,
        name: ValidDatasetNames_LT,
        dataset_root: str,
        load_img: bool = False,
    ):
        super().__init__(name, dataset_root, VOTLTVideo)
        with open(os.path.join(dataset_root, name + ".json"), "r") as f:
            meta_data: dict[str, Any] = json.load(f)
