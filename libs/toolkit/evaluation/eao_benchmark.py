...
class EAOBenchmark(Benchmark):
    ...
    def eval(self, eval_tracker:Optional[str]=None, eval_variant:Optional[str]=None):
        def get_result(
            tracker_name: str,
            variant: str,
        ):
            return self._calculate_eao(tracker_name, variant, self.tags)
    ...
    def _calculate_eao(self, tracker_name:str, variant:str, tags:List[str]):
        all_overlaps = []
        all_failures = []
        video_names = []
        gt_traj_length = []
        ...
        for video in self.dataset:
            ...
            if tracker_name not in video.pred_trajs:
                tracker_trajs = video.load_tracker(self.dataset.tracker_path, tracker_name, variant, False)
            else:
        ...
        for tag in tags:
            ...
            # print(f'\texpected_overlaps[is_valid]:', expected_overlaps[is_valid])
            # print(f'\texpected_overlaps[is_valid].shape:', expected_overlaps[is_valid].shape)
            # print(f'\tweight[is_valid]:', weight[is_valid])
            # print(f'\tweight[is_valid].shape:', weight[is_valid].shape)
            # print(f'\tsum(expected_overlaps[is_valid] * weight[is_valid]):', np.sum(expected_overlaps[is_valid] * weight[is_valid]))
