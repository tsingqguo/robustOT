...
from pyotp.utils import silent_nanmean
from . import region
...
def success_overlap(gt_bb: npt.NDArray[np.int64], result_bb: npt.NDArray[np.float64], n_frame: int):
    thresholds_overlap = np.arange(0, 1.05, 0.05)
    success = np.zeros(len(thresholds_overlap))
    iou = np.ones(len(gt_bb)) * (-1)
    # mask = np.sum(gt_bb > 0, axis=1) == 4 #TODO check all dataset
    mask: List[bool] = np.sum(gt_bb[:, 2:] > 0, axis=1) == 2
