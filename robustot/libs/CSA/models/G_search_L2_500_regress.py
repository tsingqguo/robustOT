...
class SiamRPNPP:
    """dummy class; used in training process"""
...
# class GsearchL2500regressModel(Base_L2_500_Search):
# class GsearchL2500regressModel(BaseModel_CSA):
# class CSAModel(BaseModel_CSA):
class CSAModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        """
        self.template = (
            input[0].squeeze(0).cuda()
        )  # pytorch tensor, shape=(1,3,127,127)

        self.search_clean255 = (
    ...
    def forward(self, target_sz=(255, 255)):
        ...
        """resize to (512,512)"""
        search512_clean1 = F.interpolate(
            self.search_clean1,
            size=(512, 512),
            mode="bilinear",
            align_corners=False,
        )
        search512_adv1 = search512_clean1 + self.netG(
            search512_clean1
        )  # Residual form: G(A)+A
        """Then crop back to (255,255)"""
        self.search_adv1 = F.interpolate(
            search512_adv1,
            size=target_sz,
            mode="bilinear",
            align_corners=False,
    ...
    def transform(self, patch_clean1, target_sz=(255, 255)):
        """resize to (512,512)"""
        patch512_clean1 = F.interpolate(
            patch_clean1, size=(512, 512), mode="bilinear", align_corners=False
        )
        patch512_adv1 = patch512_clean1 + self.netG(
            patch512_clean1
        )  # Residual form: G(A)+A
        patch_adv1 = F.interpolate(
            patch512_adv1, size=target_sz, mode="bilinear", align_corners=False
        )
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        self.loss_G_L2 = (
            self.criterionL2(self.search_adv1, self.search_clean1)
            * self.weight_L2
        )
        ...
        if num_attention > 0:
            ...
            self.loss_cls = (
                torch.mean(
                    torch.clamp(
                        score_map_adv_att[:, 1] - score_map_adv_att[:, 0],
                        min=self.cls_margin,
                    )
                )
                * self.weight_cls
            ...
            self.loss_reg = (
                torch.mean(
                    torch.clamp(reg_adv_att[0, :], min=self.side_margin1)
                )
                + torch.mean(
                    torch.clamp(reg_adv_att[1, :], min=self.side_margin2)
                )
            ) * self.weight_reg
    ...
    def optimize_parameters(self):
        ...
        with torch.no_grad():
            ...
            self.score_maps_clean = self.siam.get_heat_map(
                self.search_clean255, softmax=True
            )  # (5HWN,),with softmax
        # 2. adversarial attack with GAN
        self.forward()  # compute fake image
        # 3. predict with adversarial template
        self.score_maps_adv, self.reg_res_adv = self.siam.get_cls_reg(
            self.search_adv255, softmax=False
