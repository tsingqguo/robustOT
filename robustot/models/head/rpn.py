...
from msot.libs.pysot.core.xcorr import xcorr_fast, xcorr_depthwise
...
class RPN(nn.Module):
    ...
    def forward(
        self, z_f: torch.Tensor, x_f: torch.Tensor
...
class DepthwiseXCorr(nn.Module):
    def __init__(
        ...
        self.conv_kernel = nn.Sequential(
            nn.Conv2d(
                in_channels, hidden, kernel_size=kernel_size, bias=False
            ),
    ...
    def forward(self, kernel, search):
        ...
        out = self.head(feature)  # [B, 256, 25, 25]
        return out
...
class MultiRPN(RPN):
    def __init__(
        self, anchor_num: int, in_channels: list[int], weighted=False
    ):
    ...
    def forward(self, z_fs: list[torch.Tensor], x_fs: list[torch.Tensor]):
        cls = []
        loc = []
        ...
        def avg(lst: list[torch.Tensor]):
            return sum(lst) / len(lst)

        def weighted_avg(lst: list[torch.Tensor], weight) -> torch.Tensor:
            s = 0
            for i in range(len(weight)):
                s += lst[i] * weight[i]
            return s  # type: ignore # TODO:
        ...
        if self.weighted:
            ...
            loc = weighted_avg(loc, loc_weight)
        else:
            cls = avg(cls)
            loc = avg(loc)
